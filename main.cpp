...
int main(int argc, char **argv) {
  try {
    if (argc > 1 && std::string(argv[1]) == "--help") {
      std::cout
          << "resonator [steps=200] [side=100] [memory=0.4] [dt=0.01]\n"
    ...
    Parameters p;
    if (argc > 3)
      p.memory = std::stod(argv[3]);
    if (argc > 4)
      p.dt = std::stod(argv[4]);
    if (steps < 0)
      throw std::invalid_argument("Steps cannot be negative");
    Simulation simulation(side, 42, 1000, p);
    std::cout << "time,positive,negative,peak_density,density_rms,limited_"
                 "fraction,steps,arena_mib\n";
    auto report = [&]() {
      double stats[8];
      simulation.statistics(stats);
      for (int i = 0; i < 8; ++i)
        std::cout << (i ? "," : "") << std::setprecision(12) << stats[i];
      std::cout << '\n';
    };
    report();
